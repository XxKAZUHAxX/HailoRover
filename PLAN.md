...
- **Venv strategy** — the server runs inside `venv_hailo_apps` (Trixie, Python 3.13); server requirements + hailo-layer are pip-installed into it. `opencv-python` is replaced by `opencv-python-headless` (expected). The hailo-apps clone lives **inside this repo** (`hailo-apps/`, gitignored) so the runtime is self-contained. `setup.sh` at the repo root is the single entry point, dual-mode: `bash setup.sh` runs the full one-shot install (install.sh → venv repair → server deps → hailo-layer), `source setup.sh` activates the venv from anywhere in the repo (install-and-activate on first source, fast activation after).
